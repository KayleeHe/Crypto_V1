...
from CommIf.CodeHandle import CodeConvert
from ApiData.Tushare import basic_code_list

class ManageCodeTable(CodeConvert):
    # A股指数代码
    indexes = OrderedDict([
                        ('上证指数', 'sh.000001'),
                        ('深证成指', 'sz.399001'),
    ...
    def update_stock_code(self):
        # 生成A股全市场股票名称-股票代码映射代码表 self._stock_codes_table {'平安银行': 'sz.000001', ...}
        ...
        if code_group.empty != True:
            ...
            for index, row in code_group.iterrows():
                # 股票代码表统一使用tushare xxxxxx.SH/xxxxxx.SZ 格式
                #num, sym = row["ts_code"].lower().split(".")
                #bs_code = sym + "." + num
                self._stock_codes_table[row["name"]] = row["ts_code"]
    ...
    def specific_stock_code(self, date_lim, st_rm)->dict:

        # A股全市场股票代码
        self.syslog.re_print("从TuSharePro获取指定要求的股票基本信息...\n")

        code_group = basic_code_list(['ts_code', 'name', 'area', 'list_date'])

        if code_group.empty != True:

            code_name = {}
            # 剔除出新股和次新股 即只考虑2017年1月1日以前上市的股票
            code_group = code_group[code_group['list_date'].apply(int).values < date_lim]  # 转换为int后筛选

            if st_rm == True:
                code_group.dropna(subset=['name'], inplace=True)
                code_group = code_group[code_group['name'].apply(lambda x: x.find('ST') < 0)]
    ...
    def get_name(self, code):
        ...
        for name, code_ in self.stock_all_codes.items():
            if code_ == self.conv_ts_code(code):
                return name
