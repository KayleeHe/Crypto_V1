...
class ManageSelfPool(CodeConvert):
    ...
    def load_pool_group(self):
        # 加载自选股票池-组合分析
        return self.load_self_pool()["组合"]

    def update_increase_st(self, item, new_code):
        # 增量更新
        st_code = self.load_self_pool()
        st_code[item].update(self.conv_ts_codes(new_code))
    ...
    def update_replace_st(self, new_code):
        ...
        st_code = self.load_self_pool()
        st_code['股票'].clear()
        st_code['股票'].update(self.conv_ts_codes(new_code))
        self.save_self_pool(st_code)
...
class ManageTradePool():
    ...
    def update_increase_st(self, new_stock):
        # 增量更新
        st_codes = self.load_total_info()
        st_codes.update(new_stock)
        self.save_trade_pool(st_codes)
        self.syslog.re_print(f"增量更新{self.poolname}成功...\n")

    def update_replace_st(self, new_name):
        # 完全替换
        st_codes = self.load_self_pool()
        st_codes.clear()
        st_codes.update(new_name)
        self.save_trade_pool(st_codes)
