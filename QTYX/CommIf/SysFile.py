...
class Base_File_Oper():
    ...
    @staticmethod
    def write_tushare_token(cont):
        # 设置token
        with open(Base_File_Oper.rel_path + 'token.txt', 'w', encoding='utf-8') as f:
            f.write(cont)

    @staticmethod
    def save_patten_analysis(df_basic, filename):
        # 存储形态识别分析结果
        df_basic.to_csv(Base_File_Oper.rel_path +"全市场选股结果/"+filename +".csv", columns=df_basic.columns, index=True, encoding='GBK')

    def load_patten_analysis(filename):
        # 加载形态识别分析结果
        df_basic = pd.read_csv(Base_File_Oper.rel_path + "全市场选股结果/"+filename +".csv", parse_dates=True, index_col=0, encoding='GBK')
        return df_basic

    @staticmethod
    def save_self_pool(df_codes, filename):
        # 存储形态识别分析结果
        df_codes.to_csv(Base_File_Oper.rel_path +filename +".csv", columns=df_codes.columns, index=True, encoding='GBK')
