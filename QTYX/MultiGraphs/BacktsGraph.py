...
class BacktsGraphIf(MplTypesDraw):
    ...
    def cash_profit_graph(stock_dat, sub_graph, para_dat = None):
        ...
        print(f'亏损次数:{loss_count}, 盈利次数:{win_count}')
        print(f'胜率:{round(win_count / (win_count + loss_count) * 100, 2)}') if (win_count + loss_count) != 0 else print(f'胜率: NA')

        print(f'平均亏损:{round((loss_profit / loss_count), 2)}%') if loss_count != 0 else print(f'平均亏损:{0}')
        print(f'平均盈利:{round((win_profit / win_count), 2)}%') if win_count != 0 else print(f'平均盈利:{0}')
