...
class CollegeTreeListCtrl2(wx.gizmos.TreeListCtrl):

    def __init__(self, parent=None, store_path="", id=-1, pos=(0, 0), size=wx.DefaultSize,
                 style=wx.TR_DEFAULT_STYLE | wx.TR_FULL_ROW_HIGHLIGHT):
    ...
    def refDataShow(self, newDatas):
        ...
        down_stock_list = self.init_down_path()

        if self.root != None:
            self.DeleteAllItems()

        if down_stock_list != None:
            self.root = self.AddRoot(self.store_path)
            self.SetItemText(self.root, self.store_path, 0)
            ...
            for cityID in down_stock_list: # 将本地csv文件填充整个树
                ...
                self.SetItemText(child, newDatas.get(cityID[0:6]+"."+cityID[6:8], ''), 1)
                self.SetItemImage(child, 0, which=wx.TreeItemIcon_Normal) # wx.TreeItemIcon_Expanded
                self.Expand(self.root)
