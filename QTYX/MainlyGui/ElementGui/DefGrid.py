...
class GridTable(wx.grid.Grid):
    ...
    def __init__(self, parent=None, nrow=30, ncol=10):
    ...
    def SetTable(self, data, tran_col):
        ...
        if isinstance(data, dict):
            ...
            for m_k, m_v in data.items():
                ...
                self.SetCellValue(id, 0, m_k)
                self.SetCellValue(id, 1, m_v)
                id = id + 1
        else:
            # dataframe 类型
            df_data = data
