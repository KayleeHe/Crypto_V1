...
class EventHandle:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.event_task['set_csvst_pool'] = self.set_csvst_pool

    # 代码格式转换
    def conv_code(self, x):
        # 转换为sz.xxxxxx/sh.xxxxxx格式

        if x.find('.') == -1: # 正常代码格式
            return "sh." + x if x[0] == '6' else "sz." + x  # xxxxxx 转baostock格式

        elif x.find(".") == 6: # xxxxxx.SH 或 .SZ
            num, sym = x.lower().split('.')
            return sym + "." + num

        elif x.find('.') == 2:  # baostock的代码格式
            return x
        else:
            return x
    ...
    def get_stock_dat(self, **kwargs):
        ...
        try:
            df = bs_k_data_stock(self.conv_code(st_code),
    ...
    def get_csvst_pool(self, **kwargs):
        ...
        try:
            ...
            if '股票代码' in csv_df.columns: # 兼容通达信
                csv_df['股票代码'] = csv_df['股票代码'].apply(self.conv_code)
    ...
    def set_csvst_pool(self, **kwargs):
        # 设置csv文件输出股票池

        # 传递参数
        set_codes = kwargs["set_codes"]
        df_codes = pd.DataFrame({'股票名称': list(set_codes.keys()), '股票代码': list(set_codes.values())})
        try:
            Base_File_Oper.save_self_pool(df_codes, f"我的选股票池-{datetime.datetime.now().strftime('%Y%m%d-%H:%M')}")
    ...
    def get_cash_flow(self, **kwargs):
        ...
        try:
            ...
            df = bs_cash_flow_stock(self.conv_code(st_code), pre_fst_year, pre_fst_quarter)
            if df.empty == True:
                # 获取上上一季度的现金流量数据
                df = bs_cash_flow_stock(self.conv_code(st_code), pre_sec_year, pre_sec_quarter)
